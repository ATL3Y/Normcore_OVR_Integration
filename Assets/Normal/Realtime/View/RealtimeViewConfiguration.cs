...
namespace Normal.Realtime {
    public class RealtimeViewConfiguration {
        ...
        public static void _HierarchyDidChange() {
            ...
            List<RealtimeView> rootSceneViews = new List<RealtimeView>();
            ...
            // Highlight any root scene views with duplicate UUIDs
            foreach (RealtimeView rootSceneView in rootSceneViews) {
                SerializedObject rootSceneViewSerializedObject = new SerializedObject(rootSceneView);
                rootSceneViewSerializedObject.Update();
                SerializedProperty sceneViewUUIDProperty = rootSceneViewSerializedObject.FindProperty("_sceneViewUUID");

                foreach (RealtimeView otherRootSceneView in rootSceneViews) {
                    if (rootSceneView == otherRootSceneView)
                        continue;

                    SerializedObject otherRootSceneViewSerializedObject = new SerializedObject(otherRootSceneView);
                    otherRootSceneViewSerializedObject.Update();
                    SerializedProperty otherSceneViewUUIDProperty = otherRootSceneViewSerializedObject.FindProperty("_sceneViewUUID");

                    byte[]      sceneViewUUID = GetSceneViewUUIDAsByteArray(     sceneViewUUIDProperty);
                    byte[] otherSceneViewUUID = GetSceneViewUUIDAsByteArray(otherSceneViewUUIDProperty);
                    if (sceneViewUUID == null || sceneViewUUID.Length == 0 || otherSceneViewUUID == null || otherSceneViewUUID.Length == 0)
                        continue;

                    if (sceneViewUUID.SequenceEqual(otherSceneViewUUID)) {
                        Debug.LogError("RealtimeView: Duplicate UUID found in scene. This can happen if you duplicate a scene file and additively load both copies. If you plan to additively load both scenes, make sure to reset the UUID on at least one of the RealtimeViews under Advanced Settings.", rootSceneView);
                    }
                }
            }
        }
